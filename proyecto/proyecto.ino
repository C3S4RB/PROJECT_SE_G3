...
const char* ssid = "HOMEBORJA";      
const char* password = "calebnicolas1";  
/* ======================================== */

/* ======================================== Variables for telegram bot tokens. */
String BOTtoken = "6761503651:AAEaX-1dgXfUH-ELe11YoC_gq_BzHmoMek0";  
/* ======================================== */


String CHAT_ID = "1023220106";
/* ======================================== */
...
void FB_MSG_is_photo_send_successfully (bool state) {
  ...
  if(state == false) {
    send_feedback_message += "Desde la  ESP32-CAM :\n\n";
    send_feedback_message += "ESP32-CAM falló al enviar foto.\n";
    send_feedback_message += "Sugerencia :\n";
    send_feedback_message += "- Itentar de nuevo.\n";
    send_feedback_message += "- Resetear ESP32-CAM.\n";
    send_feedback_message += "- Cambiar FRAMESIZE (see Drop down frame size in void configInitCamera).\n";
    ...
    if(boolPIRState == true) {
      ...
      send_feedback_message += "Envio de foto exitoso. \n\n";
      send_feedback_message += "/start : para ver todos los comandos.";
      bot.sendMessage(CHAT_ID, send_feedback_message, ""); 
    }
    if(sendPhoto == true) {
...
void habilitar_captura_foto_flash(bool state) {
...
bool captura_foto_flash_estado() {
...
void habilitar_captura_foto_PIR(bool state) {
...
void handleNewMessages(int numNewMessages) {
  ...
  for (int i = 0; i < numNewMessages; i++) {
    ...
    if (text == "/start") {
      ...
      send_feedback_message += "/habilitar_captura_foto_flash : takes a new photo with LED FLash\n";
      send_feedback_message += "/deshabilitar_captura_foto_flash : takes a new photo without LED FLash\n";
      send_feedback_message += "/habilitar_captura_foto_PIR : takes a new photo with PIR Sensor\n";
      send_feedback_message += "/deshabilitar_captura_foto_pir : takes a new photo without PIR Sensor\n\n";
      send_feedback_message += "Setting status :\n";
      if(captura_foto_flash_estado() == ON) {
        send_feedback_message += "- Capture Photo With Flash = ON\n";
      }
      if(captura_foto_flash_estado() == OFF) {
        send_feedback_message += "- Capture Photo With Flash = OFF\n";
      }
      if(captura_foto_pir_estado() == ON) {
        send_feedback_message += "- Capture Photo With PIR = ON\n";
      }
      if(captura_foto_pir_estado() == OFF) {
    ...
    // The condition if the command received is "/habilitar_captura_foto_flash".
    if (text == "/habilitar_captura_foto_flash") {
      habilitar_captura_foto_flash(ON);
      send_feedback_message += "Desde la ESP32-CAM:\n\n";
      if(captura_foto_flash_estado() == ON) {
    ...
    // The condition if the command received is "/deshabilitar_captura_foto_flash".
    if (text == "/deshabilitar_captura_foto_flash") {
      habilitar_captura_foto_flash(OFF);
      send_feedback_message += "Desde la ESP32-CAM:\n\n";
      if(captura_foto_flash_estado() == OFF) {
    ...
    // The condition if the command received is "/habilitar_captura_foto_PIR".
    if (text == "/habilitar_captura_foto_PIR") {
      habilitar_captura_foto_PIR(ON);
      send_feedback_message += "Desde la ESP32-CAM:\n\n";
      if(captura_foto_pir_estado() == ON) {
    ...
    // The condition if the command received is "/deshabilitar_captura_foto_pir".
    if (text == "/deshabilitar_captura_foto_pir") {
      habilitar_captura_foto_PIR(OFF);
      send_feedback_message += "Desde la ESP32-CAM:\n\n";
      if(captura_foto_pir_estado() == OFF) {
...
String sendPhotoTelegram() {
  ...
  /* ::::::::::::::::: Turns on LED FLash if setting is "habilitar_captura_foto_flash(ON);". */
  if(captura_foto_flash_estado() == ON) {
    LEDFlash_State(ON);
  }
  delay(1500);
...
void setup(){
  ...
  *//* ::::::::::::::::: */
  ...
  Serial.println();
  Serial.println("El sensor pir ha sido estabilizado");
  ...
  if(PIR_Sensor_is_stable == false) {
    if(millis() > lastTime_countdown_Ran + countdown_interval_to_stabilize_PIR_Sensor) {
      ...
      if(countdown_to_stabilize_PIR_Sensor == 0) {
        ...
        Serial.println();
        Serial.println("------------");
        Serial.println("El  tiempo de estabilización del Sensor PIR ha sido completado..");
        Serial.println("El sensor PIR ya puede trabajar.");
        Serial.println("------------");
        String send_Status_PIR_Sensor = "";
        send_Status_PIR_Sensor += "Desde la ESP32-CAM :\n\n";
        send_Status_PIR_Sensor += "El  tiempo de estabilización del Sensor PIR ha sido completado.\n";
        send_Status_PIR_Sensor += "El sensor PIR ya puede trabajar.";
